...
boolean_import_library = False
t_sleep = 60 * 1 #secondes
retry_main = 5 
retry_tmp = 0 
name_notebook = "train_v1"

while boolean_import_library == False and retry_main != retry_tmp :
  print(f"Running import library {name_notebook} Start ...")
  try:
    ...
    boolean_import_library = True
  except Exception:
    print("Retrying %s : import library" % retry_tmp)
    retry_tmp += 1
    print(f"waits {t_sleep} seconds")
    time.sleep(t_sleep)
else :
  if boolean_import_library == True :
    print(f"Running import library {name_notebook} End ...")
    
  elif retry_main == retry_tmp :
    print(f"{name_notebook} import library Failed")

# COMMAND ----------

# MAGIC %run /Users/alexis.vincent@mousquetaires.com/DecPDV_etude/CFM_correct_dec
# MAGIC

# COMMAND ----------

#%run /Users/alexis.vincent@mousquetaires.com/DecPDV_etude/CFM_devdecpdv
...
path_dataframe = f"{class_groupe.path_train_preproc_ml}{class_groupe.label}.sav"
print(dataframe["DT_VALR"][0])

# COMMAND ----------

# dbutils.notebook.exit(path_filename_model)

# COMMAND ----------

from dateutil.relativedelta import relativedelta

debutDateHist = dataframe["DT_VALR"][0]
finDateHist = debutDateHist + relativedelta(years = 1)
date_run_train = finDateHist


# COMMAND ----------

for i in range(3):
#while finDateHist != dataframe["DT_VALR"][len(dataframe) - 1] :

  path_filename_model = main_train_model(path_dataframe, debutDateHist, finDateHist, date_run_train, class_groupe, path_models)

  finDateHist = finDateHist + relativedelta(days = 1)
  date_run_train = date_run_train + relativedelta(days = 1)